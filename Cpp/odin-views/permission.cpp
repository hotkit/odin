...
namespace {
    ...
    std::pair<boost::shared_ptr<fostlib::mime>, int> check_permission(
            ...
            const fostlib::json &full_config, const fostlib::string &path,
        ...
        if ( granted ) {
            return fostlib::urlhandler::view::execute(config["allowed"], path, req, host);
        } else {
            return fostlib::urlhandler::view::execute(config["forbidden"], path, req, host);
        }
    }
