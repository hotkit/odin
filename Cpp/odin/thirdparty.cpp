...
//                 __PRETTY_FUNCTION__,
//                 "The <VIEW_NAME> view must be wrapped by an "
...
//     <<MOCK LOGIC>>
//     user_detail = <<FUNCTION>>(access_token);
...
//     auto const user_id =
//             fostlib::coerce<f5::u8view>(user_detail[<<MAP_KEY_NAME>>]);
//     fostlib::pg::connection cnx{fostgres::connection(config, req)};

//     auto const reference = odin::reference();
//         f5::u8view const app_id = req.headers()["__app"].value();
//     auto thirdparty_user = odin::thirdparty_user::app_credentials(
//                     cnx, thirdparty, user_id, app_id);
//     logger(thirdparty + "_user", thirdparty_user);
//     fostlib::string identity_id;
//     fostlib::string app_user_id;
//     if (thirdparty_user.isnull()) {
//         /// We've never seen this thirdparty identity before,
...
//         /// their thirdparty a/c to their pre-existing identity.
...
//                     cnx, thirdparty, user_id, app_id);
//             app_user_id = fostlib::coerce<fostlib::string>(
//                     thirdparty_user["app_user"]["app_user_id"]);
//         } catch (const pqxx::unique_violation &e) {
